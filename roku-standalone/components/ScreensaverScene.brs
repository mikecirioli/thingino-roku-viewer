...
sub init()
    ...
    m.cycleTimer.observeField("fire", "nextCamera")
...
sub nextCamera()
    ...
    idx = (m.currentIdx + 1) mod m.cameras.count()
    startCamera(idx)
end sub
...
sub refreshSnapshot()
    ...
    m.pendingBuffer = if m.frontPoster = "a" then "tmp:/saver_b.jpg" else "tmp:/saver_a.jpg"
