...
<vote_number>46</vote_number>
<vote_date>March 4, 2026, 04:08 PM</vote_date>
<modify_date>March 4, 2026, 05:38 PM</modify_date>
<vote_question_text>On the Motion to Discharge S.J.Res. 104</vote_question_text>
...
<vote_result_text>Motion to Discharge Rejected (47-53)</vote_result_text>
<question>On the Motion to Discharge</question>
<vote_title>Motion to Discharge S.J. Res. 104</vote_title>
<majority_requirement>1/2</majority_requirement>
<vote_result>Motion to Discharge Rejected</vote_result>
<document>
<document_congress>119</document_congress>
<document_type>S.J.Res.</document_type>
<document_number>104</document_number>
<document_name>S.J.Res. 104</document_name>
<document_title>A joint resolution to direct the removal of United States Armed Forces from hostilities within or against the Islamic Republic of Iran that have not been authorized by Congress.</document_title>
...
xml2js.parseString(xmlData, (err, result) => {
  ...
  members.forEach(member => {
    ...
    const vote = member.vote_cast[0];
    
    const subject = subjects.find(s => s.name.includes(lastName) && s.state === state && s.description.includes('Senator'));
    
    if (subject) {
      actions.push({
        id: `sen-iran-res-104-${subject.id}`,
        subjectId: subject.id,
        date: "2026-03-04",
        topic: "Foreign Policy",
        title: `Voted ${vote} on Iran War Powers Resolution`,
        description: `Voted ${vote} on the Motion to Discharge S.J.Res. 104: A joint resolution to direct the removal of United States Armed Forces from hostilities within or against the Islamic Republic of Iran that have not been authorized by Congress.`,
        sourceUrl: "https://www.senate.gov/legislative/votes_new.htm"
      });
    } else {
      console.log(`Could not find match for ${lastName} (${state})`);
    }
  });

  const existingActions = JSON.parse(fs.readFileSync('src/data/actions.json', 'utf8'));
  const filteredExisting = existingActions.filter(a => a.id !== 'action-sen-iran-2026');
  const allActions = [...filteredExisting, ...actions];
  
  fs.writeFileSync('src/data/actions.json', JSON.stringify(allActions, null, 2));
  console.log(`Successfully mapped and added ${actions.length} votes.`);
});
